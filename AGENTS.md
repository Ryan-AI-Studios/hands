...
`hands do-task --goal <text>` is an optional client of the shipped primitives (default `grok-4.6`, `HANDS_XAI_API_KEY` / `XAI_API_KEY`). No auto-confirm. A fence refuse or challenge yield stops the loop. Live xAI is not a compile gate.
