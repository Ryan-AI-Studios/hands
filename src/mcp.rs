...
use crate::logs;
use crate::observe::{ObserveRequest, ObserveScope, ObserveView, observe};
...
impl HandsServer {
    ...
    #[tool(
        description = "Scroll the mouse wheel (signed dy notches; negative = toward the user). Default target is the foreground client centre (moves the cursor there). Optional --x/--y / element / grid hovers first. No FG window is ok:false. Same client-rect guard on targeted scroll."
    )]
    fn scroll(
        &self,
...
fn observe_envelope(params: ObserveParams) -> Result<String, HandsError> {
    ...
    let envelope = observe(ObserveRequest {
        ...
        scope,
        fg_preview: params.fg_preview.unwrap_or(false),
    })?;
    crate::observe::serialize_mcp_envelope_opts(
        &envelope,
        include_screenshot_path,
        params.fg_preview.unwrap_or(false),
    )
}
