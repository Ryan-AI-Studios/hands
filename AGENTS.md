...
- No secrets in git
- No Playwright, CDP, `--remote-debugging-port`, or CAPTCHA solvers **on daily Chrome** (research `--identity research` + `challenge --solve` is the exception). Ears (`listen`) must not auto-solve.
