...
`hands mcp` serves stdio MCP (`observe`, `click`, `hover`, `type`, `key`, `scroll`, `wait_settle`, `stop`, `confirm`, `attach`, `pick`, `ground`, `challenge`, `logs`). `hands observe [--detail dom] [--session-id <id>]` prints a compact observe envelope (screenshot **path**, 100px grid descriptor, UIA map, Chrome `chr:` ids when the native host is connected, capped extract, `challenge` status). Image bytes are never inlined. Envelope field `chrome_connected` is true when a host or `HANDS_CHROME_SNAPSHOT` fixture is present. **`observe` does not launch Chrome.** **`observe` does not call Gemma.**
