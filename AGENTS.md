...
`hands mcp` is the stdio MCP server (`observe` plus `click` / `hover` / `type` / `key` / `scroll` / `wait_settle` / `stop` / `confirm` / `attach` / `logs`). `hands observe [--detail dom] [--session-id <id>]` prints the compact observe envelope on stdout (UIA + Chrome `chr:` when connected; `chrome_connected` on the envelope). **`observe` does not launch Chrome.** Input subcommands print a compact actuate envelope (`ok`, `frozen`, `retried`, `settled`, `foregrounded`, optional `fence`). Pause/Break and MCP `stop` halt injection and wipe session/once allows; persist stays; **logs stay** (`%LOCALAPPDATA%\hands\logs\`, override `HANDS_LOGS_DIR`). `type` logs `len` only; observe logs path + counts, not `main_text` / url / DOM. `hands logs --session-id <id>` tails (does not mint); `--list` lists files. `desk` is reserved; `desk.jsonl` is unbounded (no rotation). CLI `stop` still clears session allows; injection is a no-op without a live MCP lease. The confirm fence lives in this binary: refuse gated `click` / `key enter` without an allow; after a refuse the harness must call `confirm` then retry (Grok is always-approve). `hands confirm`, `hands attach`, and `hands logs` do not install the desk lease.
