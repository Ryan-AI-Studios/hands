...
Selector: `hwnd:` (optional `0x`) is deterministic; digit-only is pid; otherwise a unique case-insensitive substring against the **full untruncated** titled inventory. The envelope list is capped (≤12, title ≤40) with `windows_total` / `windows_truncated`. The sidecar holds the full inventory. Display caps do not affect matching.
