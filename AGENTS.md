...
`hands do-task --goal <text>` is an optional client of the shipped primitives (default `grok-4.6`, `HANDS_KEY` / `HANDS_XAI_API_KEY` / `XAI_API_KEY`). No auto-confirm. A fence refuse or challenge yield stops the loop. Closing JSONL `error` is only a real failure message, not `done` / `fence` / `yield` / other checkable stops. Live xAI is not a compile gate.
