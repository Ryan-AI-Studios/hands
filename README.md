...
`hands sequence --steps-json <path>` runs a fixed script of up to 8 allowlisted steps and aborts on the first failed prerequisite. Not `do_task`. Not confirm-gated as a whole. No *new* inter-step dwell (existing hover/scroll 100 ms dwell unchanged). Do not live-drive in CI.
