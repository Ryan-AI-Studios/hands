...
`hands challenge [--status] [--watch] [--observe-path <path>] [--session-id <id>]` reports the in-process challenge episode. Interstitial titles and origin `cdn-cgi` set `challenge.present`; wait (`wait_settle` / `--watch`); do not click “Just a moment…”. A visible “are you human” UI can be tried as computer-use for **two observe-cycles that used actuation**. After that, actuation refuses (`yielded`) with **no SendInput**. Resume only when the UI is gone. Idle is not resume. Not a solver. A yield-refused hover, like click, does not update the process-local last-target slot; standalone `wait_settle` is still the foreground window.
